    ...
    <div class="movie-info border-b border-gray-800">
        <div class="container mx-auto px-4 py-16 flex flex-col md:flex-row">
            ...
            <div class="md:ml-24">
                ...
                <p class="text-sm text-gray-300 mt-1">
                    <span class="">Date of Birth: </span>{{ actor.birthday | moment }}
                </p>

                <p class="text-sm text-gray-300 mt-1">
                    <span class="">Place of Birth: </span>{{ actor.place_of_birth }}
                </p>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8">
                    <div class="mt-4" v-for="cast in actor.combined_credits.cast.slice(0, 5)" :key="cast.id">
                        <a href="#"><img :src="`https://image.tmdb.org/t/p/w500/${cast.poster_path}`" alt="poster" class="hover:opacity-75 transition ease-in-out duration-150"></a>
        ...
        <div class="border-t border-gray-800">
            <div class="container mx-auto px-4 py-16">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8">
                    <div class="mt-4" v-for="cast in actor.combined_credits.cast.slice(0, 20)" :key="cast.id">
                        <a :href="`/movies/${cast.id}`"><img :src="`https://image.tmdb.org/t/p/w500/${cast.poster_path}`" :alt="cast.title" class="hover:opacity-75 transition ease-in-out duration-150"></a>
                        <a href="#" class="text-sm leading-normal block text-gray-400 hover:text-white mt-1">{{ cast.title }}</a>
                    </div>
                </div>
            </div>
...
<script>
    export default {
        ...
        methods: {
            fetchActor(id) {
                axios.get(`https://api.themoviedb.org/3/person/${id}?api_key=${this.apiKey}&append_to_response=combined_credits`).then((res) => {
                    this.actor = res.data;
